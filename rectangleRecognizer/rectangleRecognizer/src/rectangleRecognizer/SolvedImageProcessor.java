...
public class SolvedImageProcessor {
	...
	public SolvedImageProcessor()
	{
		File[] fileList = SolvedImageProcessor.dir.listFiles();
		...
		else
		{
			...
			SolvedImageProcessor.numberImages = fileList.length;
...
//		        if(pixelColor.getRed() > 150 && pixelColor.getBlue() < 150 && pixelColor.getGreen() < 150)
		        if(pixelColor.getRed() > pixelColor.getBlue() + pixelColor.getGreen() && pixelColor.getRed()>100)
		        {
		        	numRedPixels++;
...
//		    			toProcess.setRGB(i, j, Color.green.getRGB());
		    		...
		    		else if(highestPoint.y == j && highestPoint.x < i)
		    		{
		    			highestPoint = new Point(i, j);
		    		}
		    		if(lowestPoint.y < j)
...
//		    			toProcess.setRGB(i, j, Color.green.getRGB());
		    		...
		    		else if(rightMostPoint.x == i && rightMostPoint.y < j)
		    		{
		    			rightMostPoint = new Point(i, j);
		    		}
		    	}
...
//		toProcess.setRGB(highestPoint.x, highestPoint.y, Color.green.getRGB());
		polyPoints[1] = lowestPoint;
		polyPoints[2] = highestPoint;
		polyPoints[3] = leftMostPoint;
		polyPoints[0] = rightMostPoint;
...
//	    System.out.println(blobSize);
        ...
        imageColors[i][j] = new Color(colorInt);
        Color pixelColor = new Color(colorInt);
        if(pixelColor.getRed() <= pixelColor.getBlue() + pixelColor.getGreen() || pixelColor.getRed()<100) // red
	        return;
